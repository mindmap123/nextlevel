...
export function GradientWaveText({
  ...
  const stops = useMemo(() => {
    const arr: string[] = [];
    const baseColor = "var(--gradient-wave-base, rgb(100,116,139))";
  ...
  // Start at 0 so colored bands appear immediately in the center
  const INITIAL_GI = 0;

  useEffect(() => {
    const node = elRef.current;
    if (node) node.style.setProperty("--gi", String(INITIAL_GI));
  }, []);
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      if (shouldAnimate) {
        ...
        if (cycles === 0) {
          if (next >= RANGE) next = INITIAL_GI + ((next - INITIAL_GI) % (RANGE - INITIAL_GI));
          tRef.current = next;
