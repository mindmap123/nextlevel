...
- **Démo d'appel jouable** (`VoiceCallDemo.tsx`, section Agent vocal) : simulateur d'appel entrant. Auto-start au scroll-in (sauf reduced-motion) + bouton « Simuler un appel » / « Rejouer ». Machine à états idle→ringing→connected→done : sonnerie (pulse-ring), conversation client/agent en bulles révélées une à une avec indicateur de frappe, waveform active pendant l'appel, timer live, puis carte « RDV pris » qui tombe (spring). Remplace l'ancien mockup statique. But : rendre l'idée « l'agent gère un vrai appel » limpide.
- Toutes ces interactions respectent `prefers-reduced-motion` (neutralisées) et `pointer:coarse` (pas de suivi curseur).
- **Halos d'ambiance par bloc** (`SectionAura.tsx`) : chaque section sombre a un halo radial warm très subtil (opacity 0.07-0.09), teinte + position variables dans la famille de l'accent — ambre `#F59E0B` (Pour qui), corail `#FF6B3D` (Services), or `#FFB020` (Réalisations), rouge `#E8431A` (Manifeste), orange `#FF4D17` (FAQ). VoiceAgent/SeoLocal ont leur propre glow. But : faire respirer la couleur au scroll sans casser le « 1 accent ». Le texte/CTA gardent l'orange signature.
